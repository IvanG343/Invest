		...
		<div class="col-lg-12 mb-4">
			<form action="https://perfectmoney.is/api/step1.asp" method="post" target="_blank" id="no_ajax">
				...
				<div class="control-group form-group">
					<div class="controls">
						<label>Сумма:</label>
						<input type="number" min="<?php echo $tariff['min']; ?>" max="<?php echo $tariff['max']; ?>"
									 class="form-control" value="<?php echo $tariff['min']; ?>" name="PAYMENT_AMOUNT">
					</div>
				</div>
				<input type="hidden" name="PAYMENT_UNITS" value="USD">
				<input type="hidden" name="PAYMENT_ID" value="<?php echo $this->route['id'].','.$_SESSION['account']['id']; ?>">
				<input type="hidden" name="STATUS_URL"
							 value="<?php echo $_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST']; ?>/merchant/perfectmoney">
				<input type="hidden" name="PAYMENT_URL"
							 value="<?php echo $_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST']; ?>/account/profile">
